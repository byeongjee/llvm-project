...
GlobalSetOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  ...
  if (globalType != valueType) {
    bool allowDialectCovariance = false;
    Dialect &globalDialect = globalType.getDialect();
    Dialect &valueDialect = valueType.getDialect();
    if (&globalDialect == &valueDialect &&
        globalDialect.getNamespace() != "builtin") {
      allowDialectCovariance = true;
    }
    if (!allowDialectCovariance) {
      return emitError(
                 "global.set value type does not match target global type: "
                 "expected ")
